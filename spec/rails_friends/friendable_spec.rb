...
describe User, type: :model do
  ...
  describe 'association' do
    ...
    it { should have_many(:friendships).class_name('RailsFriends::Friendship').dependent(:destroy) }
    ...
    describe '#friend_request' do
      ...
      context 'when there friend requester is blocked by friendable' do
        it 'should not create Friendship' do
          ...
          end.to change(RailsFriends::Friendship, :count).by(0)
        end
      end
      ...
      context 'when user requests friendship to friend' do
        ...
        context 'if friendship does not yet exist' do
          it 'should create 2 Friendship records' do
            expect do
              user.friend_request(friend)
            end.to change(RailsFriends::Friendship, :count).by(2)
          end
          ...
          describe 'Friendship from friend to user' do
            before :each do
              user.friend_request(friend)
              @friendship = RailsFriends::Friendship.find_by(friendable_id: friend.id,
                                                             friendable_type: friend.class.base_class.name, friend_id: user.id)
            end
    ...
    describe '#block_friend' do
      ...
      context 'when friend is blocked' do
        it 'should no longer be friends' do
          ...
          expect(RailsFriends::Friendship.find_unblocked_friendship(user, friend).present?).to eq false
        end
